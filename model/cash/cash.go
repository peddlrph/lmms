...
func All(db Connection) ([]Item, float32, bool, error) {
	...
	err := db.Select(&result, fmt.Sprintf(`
		SELECT id, trans_datetime,trans_code, amount, details, created_at, updated_at, deleted_at
		FROM %v
...
// Create adds an item.
func Create(db Connection, trans_date string, trans_code string, amount string, details string) (sql.Result, error) {
	amt, _ := strconv.ParseFloat(amount, 64)

	trans_details := "AddCash: |"
	...
	cash_details := "AddCash: |"
	cash_details = cash_details + "-  Add " + amount + " to cash.|"
	//trans_details = trans_details + "-  Add " + amount + " to codes.|"
